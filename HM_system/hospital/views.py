...
def index(request):
    ...
    return render(request, 'temp/index.html',context=mydict)
...
def patient_report(request):
    reports=models.Report.objects.filter(patientId=request.user.id).count()
    print(reports)
    print(request.user.id)
...
def patient_view_reports(request):
    reports=models.Report.objects.all().filter(patientId=13,status=True)
...
def patient_upload_report(request):
    ...
    if request.method == "POST":
        ...
        report_file = request.POST.get('report', '')
